package com.markaldrich.stdaudio;
...
public final class StdAudio {
	...
	public static void init() {
		try {
			...
			// the internal buffer is a fraction of the actual
			// buffer size, this choice is arbitrary
			// it gets divided because we can't expect the buffered
			// data to line up exactly with when
			// the sound card decides to push out its samples.
			buffer = new byte[SAMPLE_BUFFER_SIZE * BYTES_PER_SAMPLE / 3];
		} catch(LineUnavailableException e) {
			System.out.println("LineUnavailableException thrown in init()");
	 ...
	 *                 if the sample is <tt>Double.NaN</tt>
	 */
	...
	/**
	 * Writes the array of samples (between -1.0 and +1.0) to standard
	 * audio. If a sample is outside the range, it will be clipped.
	 *
	 * @param samples
	 *                the array of samples to play
	 * @throws IllegalArgumentException
	 *                 if any sample is <tt>Double.NaN</tt>
	 */
